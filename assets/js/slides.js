...
let slideItems = document.querySelector('[data-slide="item"]')
let controlButtons
let slideInterval
...
function translateSlide({ position }) {
    ...
    slideList.computedStyleMap().transform = `translateX(${position}px)`
}
...
function setVisibleSlide({ index, animate }) {
    if(index === 0 || index === slideItems.lenght - 1) {
...
function createSlideClones() {
    ...
    firstSlide.dataset.index = slideItems.lenght

    const secondSlide = slideItems[1].cloneNode(true)
    secondSlide.classList.add('slide-cloned')
    secondSlide.dataset.index = slideItems.lenght + 1

    const lastSlide = slideItems[slideItems.lenght - 1].cloneNode(true)
    lastSlide.classList.add('slide-cloned')
    lastSlide.dataset.index = -1

    const penultimateSlide = slideItems[slideItems.lenght - 2].cloneNode(true)
...
function onControlButtonClick(index) {
    ...
    function onSlideListTransitionEnd() {
        ...
        if(slideItem.classList.contains('slide-cloned') && Number(slideItem.dataset.index) < 0) {
            setVisibleSlide({ index: slideItems.lenght - 3, animate: false })
        }
    }
}
